...
def make_filters_from_source(coca, initial=690):
    ...
    width = interp1d(centers, widths_avg, kind="quadratic", fill_value="extrapolate")
    ...
    def func(center):
        return center - width(center) / 2 - (c1 + w1 / 2 + w1_std / 2)
    ...
    def func(center):
        return center - width(center) / 2 - (c2 + w2 / 2 + w2_std / 2)
    ...
    axes[0].set_title(f"t_exp ={CoCa.t_exp}")
    axes[0].legend()
    plt.savefig("filter_widths.png")
    plt.show()


def solve_for_widths(coca):
    M = get_mirror()
    Q = get_detector()
    S = get_solar()
    ...
    for filter_center in centers:
        def func(width):
            i = quad(integrand_up, filter_center - width / 2, filter_center + width / 2, args=(N, alpha))[
                0]
            signal = coca.G * coca.A * coca.Omega * coca.t_exp * i / (const.h * const.c) / coca.r_h ** 2
            return signal - 2 ** 14
    ...
    return centers, widths_up, widths_low
...
if __name__ == "__main__":
    CoCa = Camera()
    # make_filters_from_source(CoCa)
    c, w1, w2 = solve_for_widths(CoCa)
    plot_widths(c, w1, w2)